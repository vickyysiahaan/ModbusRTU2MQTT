...
def FLOAT32toUINT16(float32_val, big_endian=True):
    ...
    if big_endian:
        ...
        uint32_val = struct.unpack("<I", struct.pack("<f", np.float32(float32_val)))[0]
    result = INT32toUINT16(uint32_val, not big_endian)
...
def FLOAT64toUINT16(float64_val, big_endian=True):
    ...
    result = INT64toUINT16(uint64_val, not big_endian)
...
#Cek DHT
'''
a = Device('COM9', 1, 9600, 'NONE', 1, 8, BE, 0.2)
b = Device('COM9', 2, 9600, 'NONE', 1, 8, BE, 0.2)
VarName = ['Temperature', 'Humidity', 'ModbusAddress', 'Unknown']
Reg = [[0,1,2,3]]
Mul = [0.1,0.1,1,1]
print(a.read_INT16(VarName, Reg, Mul))
print(b.read_INT16(VarName, Reg, Mul))
a.write_num(2,65537,UINT32)
b.write_num(2,131074,UINT32)
'''
